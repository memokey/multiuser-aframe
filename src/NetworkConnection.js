...
class NetworkConnection {
  ...
  receivedData(fromClientId, dataType, data, name, avatarUrl) {
    if (this.dataChannelSubs[dataType]) {
      this.dataChannelSubs[dataType](fromClientId, dataType, data, name, avatarUrl);
      if(!!name) {
        if(window.positions == undefined) {
          window.positions = {};
        }
        window.positions[name] = data;
      }
    } else {
