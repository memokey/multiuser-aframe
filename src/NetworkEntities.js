...
class NetworkEntities {
  ...
  updateEntityMulti(client, dataType, entityDatas, name, avatarUrl) {
    if (NAF.options.syncSource) return;
    for (let i = 0, l = entityDatas.d.length; i < l; i++) {
      this.updateEntity(client, 'u', entityDatas.d[i], name, avatarUrl);
    }
  }

  updateEntity(client, dataType, entityData, name, avatarUrl) {
    if (NAF.options.syncSource) return;
    var networkId = entityData.networkId;
    entityData['name'] = name;
    entityData['avatarUrl'] = avatarUrl;
  ...
  receiveFirstUpdateFromEntity(entityData) {
    ...
    if (parentNotCreatedYet) {
      ...
      this.addEntityToPage(remoteEntity, parent, networkId);
    }
  }
  ...
  addEntityToPage(entity, parentId, networkId) {
    if (this.hasEntity(parentId)) {
      this.addEntityToParent(entity, parentId);
    } else {
      this.addEntityToSceneRoot(entity, networkId);
    }
  }
  ...
  addEntityToSceneRoot(el, networkId) {
    var scene = document.querySelector('a-scene');
    if(!NAF.entities.entities[networkId].firstUpdateData.avatarUrl) {
      NAF.entities.entities[networkId].firstUpdateData.avatarUrl = "/images/placeholder/avatars/avatar.png";
    }
    var tex = new THREE.TextureLoader().load(NAF.entities.entities[networkId].firstUpdateData.avatarUrl);
    tex.flipY = false; // for glTF models.
    el.querySelector('.uv-model').addEventListener('model-loaded', function (e) {
    ...
    el.querySelector('.nametag').setAttribute('text', "value: "+ NAF.entities.entities[networkId].firstUpdateData.name + "; align:center;")
